...
def eval_ns_batching(model, g, eval_set, batch_size, fanout, num_workers, use_ddp=False):
    ...
    with torch.no_grad():
        ...
        for it, (_, _, blocks) in minibatches:
            if args.mmap:
                x = data.node_feat.to(device)[blocks[0].srcdata[dgl.NID].cpu().numpy().astype(np.int32)].float()
                ys.append(data.labels.to(device)[blocks[-1].dstdata[dgl.NID].cpu().numpy().astype(np.int32)].flatten().to(torch.long))
            else:
    ...
    for run in range(args.runs):
        ...
        for epoch in range(args.n_epochs):
            ...
            for step, (input_nodes, output_nodes, blocks) in minibatches:
                if args.mmap:
                    x = data.node_feat.to(device)[blocks[0].srcdata[dgl.NID].cpu().numpy().astype(np.int32)].float()
                    y = data.labels.to(device)[blocks[-1].dstdata[dgl.NID].cpu().numpy().astype(np.int32)].flatten().to(torch.long)
                else:
                    x = blocks[0].srcdata['feat'].float()
