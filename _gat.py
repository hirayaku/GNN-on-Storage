...
class GATConv(nn.Module):
    ...
    def forward(self, feat, edge_index, perm=None):
        ...
        with graph.local_scope():
            ...
            print("attn_l")
            print(self.attn_l.get_device(), )
            self.attn_l = self.attn_l.to("cpu")
...
class _GATConvDGL(nn.Module):
    def __init__(
        ...
        self._out_feats = out_feats
        self._use_symmetric_norm = use_symmetric_norm
        self._allow_zero_in_degree = False # allow_zero_in_degree
        self._in_src_feats, self._in_dst_feats = expand_as_pair(in_feats)
        if isinstance(in_feats, tuple):
            self.fc_src = nn.Linear(self._in_src_feats, out_feats * num_heads, bias=False)
            self.fc_dst = nn.Linear(self._in_dst_feats, out_feats * num_heads, bias=False)
    ...
    def forward(self, feat, edge_index, perm=None):
        ...
        # just copy-paste the entire forward function
