...
typedef std::vector<torch::Tensor> Lists;
typedef std::pair<int, int> PairID;
typedef std::vector<PairID> PairIDList;
typedef std::vector<PairIDList> PairIDLists;
typedef std::vector<int> IDList;
typedef std::vector<IDList> IDLists;

PairIDLists randomlyAssignEdgeBucketsToBuffers(IDLists buffer_states, int num_partitions);
PairIDLists greedyAssignEdgeBucketsToBuffers(IDLists buffer_states, int num_partitions);
PairIDLists customAssignEdgeBucketsToBuffers(IDLists buffer_states, int num_partitions);
IDLists getBetaOrderingHelper(int num_partitions, int buffer_capacity);
std::tuple<Lists, Lists> convertEdgeBucketOrderToTensors(IDLists buffer_states, PairIDLists edge_buckets_per_buffer);

std::tuple<Lists, Lists> getTwoLevelBetaOrdering(int num_partitions, int buffer_capacity, int fine_to_coarse_ratio,
                                                 int num_cache_partitions, bool randomly_assign_edge_buckets);

int main() {
  int num_partitions = 8;
  int buffer_capacity = 4;
  int fine_to_coarse_ratio = 2;
  int num_cache_partitions = 0;
  bool randomly_assign_edge_buckets = true;
  auto tup = getTwoLevelBetaOrdering(num_partitions, buffer_capacity, fine_to_coarse_ratio, num_cache_partitions, randomly_assign_edge_buckets);
  auto buffer_states = std::get<0>(tup); // this is S in the paper
  auto edge_buckets_per_buffer = std::get<1>(tup); // this is X in the paper
  std::cout << "Printing buffer states S\n";
  std::cout << "-------------------------------\n";
  for (auto &state : buffer_states) std::cout << state << "\n";
  std::cout << "-------------------------------\n";
  std::cout << "Printing edge buckets X\n";
  std::cout << "-------------------------------\n";
  for (auto &ebuckets : edge_buckets_per_buffer) std::cout << ebuckets << std::endl;
  return 0;
}
...
std::tuple<Lists, Lists> getTwoLevelBetaOrdering(int num_partitions, int buffer_capacity, int fine_to_coarse_ratio,
  ...
  IDLists coarse_buffer_states = getBetaOrderingHelper(coarse_num_partitions, coarse_buffer_capacity);
  ...
  IDLists buffer_states;
  for (int i = 0; i < coarse_buffer_states.size(); i++) {
    IDList fine_buffer_state(buffer_capacity, 0);
    for (int j = 0; j < coarse_buffer_states[i].size(); j++) {
      ...
      IDList fine_partitions = IDList(start, end);
  ...
  // assign edge buckets
  PairIDLists edge_buckets_per_buffer;
  if (randomly_assign_edge_buckets) {
    edge_buckets_per_buffer = randomlyAssignEdgeBucketsToBuffers(buffer_states, num_partitions);
  } else {
    //edge_buckets_per_buffer = customAssignEdgeBucketsToBuffers(buffer_states, num_partitions);
...
std::tuple<Lists, Lists> convertEdgeBucketOrderToTensors(IDLists buffer_states, PairIDLists edge_buckets_per_buffer) {
  Lists ret_buffer_states;
  Lists ret_edge_buckets_per_buffer;
...
IDLists getBetaOrderingHelper(int num_partitions, int buffer_capacity) {
  ...
  buffer_states.emplace_back(IDList(data_ptr_, data_ptr_ + in_buffer.size(0)));
...
PairIDLists randomlyAssignEdgeBucketsToBuffers(IDLists buffer_states, int num_partitions) {
  ...
  std::cout << "Generating random numbers in parallel: " << num_threads << " threads\n";
#endif
  ...
  PairIDLists edge_buckets_per_buffer(num_buffers);
  for (int i = 0; i < num_buffers; i++) {
    edge_buckets_per_buffer[i] = PairIDList(num_edge_buckets_per_buffer[i].item<int>());
  }

  IDList indices(num_buffers, 0);
  for (int i = 0; i < num_buckets; i++) {
    int32_t src_part = all_buckets_accessor[i][0];
    int32_t dst_part = all_buckets_accessor[i][1];
    PairID pair = std::make_pair(src_part, dst_part);
...
PairIDLists greedyAssignEdgeBucketsToBuffers(IDLists buffer_states, int num_partitions) {
  PairIDLists edge_buckets_per_buffer(buffer_states.size());
