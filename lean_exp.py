...
def train(epoch):
    ...
    for data in train_loader:
        ...
        rand_label_mask = torch.rand(train_mask.shape[0], device=device) < mask_rate
        print(rand_label_mask.get_device())
        ...
        loss = custom_loss_function(out[rand_unlabelled_mask_full], data.y[rand_unlabelled_mask_full].view(-1))
        loss.backward()
        optimizer.step()

        total_loss += float(loss) * int(train_mask[rand_unlabelled_mask_full].sum())
...
def main():
    ...
    argparser = argparse.ArgumentParser("GAT implementation", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    argparser.add_argument("--tensorboard-folder", type=str, default="tb_output")
    argparser.add_argument("--gpu", type=int, default=0, help="GPU device ID.")
    argparser.add_argument("--lr", type=float, default=0.002)
    argparser.add_argument("--partition-type", choices=["random", "metis", "fennel", "fennel2"], default="fennel2")
    ...
    if part_type == "random":
        ...
        train_nids = dataset.get_idx_split()['train'] # this partitioner asks for train_nids
        partitioner = FennelLBPartitioner(dataset[0], psize=num_parts, train_nids=train_nids)
        results = partitioner.partition() # results is an IntTensor storing the partition id of each node
        ...
        dataset_formatted = get_dataset(dataset_conf['root'])
        indices = dataset_formatted.get_idx_split()
        out_feats = dataset_formatted.num_classes
        in_feats = dataset_formatted[0].x.shape[1]
    ...
    # set writer
    writer = SummaryWriter(log_dir=f"./{run_dir}/all_tricks_{datetime.now()}_{part_type}_{num_parts}_${args.batch_size}_seed_{args.seed}")
    # reset params
    best_val = 0.0
    final_train = 0.0
    final_test = 0.0
    patience = patience_cnt = 100
    ...
    print(f"Average test accuracy: {np.mean(final_test_list)} ± {np.std(final_test_list)}")
