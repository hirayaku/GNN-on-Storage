...
from datapipe.sampler_pipes import NodeSamplerDataPipe, NodeSubgraphDataPipe
from trainer import train, eval_full, eval_batch
from trainer.recorder import Logger
...
def make_blocks(data: Data, nodes, num_blocks, mode):
    ...
    return pt.scatter_append(0, assigns, nodes, return_sequence=True), assigns

def make_hb_dp(
    data: Data, node_dp: IterDataPipe, node_mask: torch.Tensor,
    fanout: list[int], **kwargs
):
    num_workers = kwargs.get('num_workers', 0)
    sg_data, sg_node_dp = NodeSubgraphDataPipe(data, node_dp, node_mask).unzip(sequence_length=2)
    # sg_node_dp
    sampler = PygNeighborSampler(fanout) if fanout else None
    sampler_dp = NodeSamplerDataPipe(sg_data, sg_node_dp, sample_fn=sampler)
    return sampler_dp
...
if __name__ == '__main__':
    ...
    from trainer.baseline import make_ns_dp
    ...
    if args.hb in ('random', 'metis', 'fennel'):
        ...
        train_dp = make_hb_dp(data, nid_dp, args.fanout, shuffle=True, batch_size=args.batch_size)
    elif args.hb == 'once':
